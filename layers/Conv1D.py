...
class Conv1D(Layer):
    ...
    def backward(self, cost_err: np.ndarray) -> np.ndarray:
        ...
        dx_ins = dilate_array(new_err, (new_err.shape[0],) + self.dx_shape, (self.strides,))

        p_x = self.last_in.shape[2] + self.kernel_size - (dx_ins.shape[2] + 1)
        ...
        #Updating weights
        main_window = np.lib.stride_tricks.as_strided(self.last_in, main_win_shape, main_strides)
    ...
    def save_to_file(self, filename: str = None) -> str | None:
        ...
        write_ret_str = f"Conv1D\u00A0{repr(self.funct)}\u00A0{self.kernel_weights.shape[0]}" + \
                        f"\u00A0{self.kernel_size}\u00A0{self.strides}\u00A0{self.padding_all}\u00A0{int(self.use_bias)}\n" + \
    ...
    def from_save(context: str, file_load: bool = False) -> 'Conv1D':
        ...
        def parse_and_return(handled_str: str):
            ...
            new_neuron = Conv1D(parse_act_info(prop_info[1]),       #Activation
