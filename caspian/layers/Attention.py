...
class Attention(Layer):
    ...
    def forward(self, q_data: np.ndarray, k_data: np.ndarray, 
        ...
        att_score = self.__softmax(qk_set / np.sqrt(self.d_embed))
        att_score = np.tril(att_score) if self.use_mask is True else att_score
    ...
    def backward(self, cost_err: np.ndarray) -> np.ndarray:
        ...
        if self.use_mask is True:
            new_err = np.tril(new_err)
        soft_grad = (self.__softmax(self.__last_score, new_err) / np.sqrt(self.d_embed))
