from caspian.layers import BatchNorm, LayerNorm, GroupNorm, RMSNorm
...
def test_groupnorm():
    ...
    with pytest.raises(InvalidDataException):
        _ = GroupNorm(3, 6, "test")

    with pytest.raises(InvalidDataException):
        _ = GroupNorm(3, 6, True, "test")

    with pytest.raises(InvalidDataException):
        _ = GroupNorm(3, 6, True, False, 0.0)
    ...
    # Affine variable presence
    layer = GroupNorm(3, 6, True, False)
    assert layer.layer_weight is not None
    assert layer.bias_weight is None

    layer = GroupNorm(3, 6, False, True)
    assert layer.layer_weight is None
    assert layer.bias_weight is not None

    l1 = GroupNorm(3, 6, True, True)
    l2 = GroupNorm(3, 6, False, False)
    ...
    # Deepcopy
    layer = GroupNorm(21, 168, True, False, 1e-9)
    layer2 = layer.deepcopy()
    assert layer2 is not layer
    assert layer2.bias_weight is None
    assert np.allclose(layer2.layer_weight, layer.layer_weight)
